...
void heap_bubble_up(heap_t *heap, int index) {
    ...
    // Bubble up while the current node's key is smaller than the parent's key
...
void heap_bubble_down(heap_t *heap, int index) {
    ...
    if (left_child_index < heap_size(heap) && heap->data[left_child_index].key < heap->data[smallest].key) {
        smallest = left_child_index;
    }

    if (right_child_index < heap_size(heap) && heap->data[right_child_index].key < heap->data[smallest].key) {
